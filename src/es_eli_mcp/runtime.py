...
Pulls a tiny static ``es-runtime.json.gz`` from this repo's GitHub Release on
first tool call so a dead endpoint can be repointed (edit runtime/es-runtime.json
...
Opt-out: set ``ES_ELI_RUNTIME_URL=""`` to disable the fetch.
"""
...
MANIFEST_URL = os.environ.get(
    "ES_ELI_RUNTIME_URL",
    "https://github.com/matematicsolutions/es-eli-mcp/releases/latest/download/es-runtime.json.gz",
)
_CACHE_NAME = "es-runtime.json"
_TIMEOUT = 15
_USER_AGENT = "es-eli-mcp-runtime (+https://github.com/matematicsolutions/es-eli-mcp)"

_runtime: dict[str, Any] | None = None


def _cache_dir() -> Path:
    env = os.environ.get("ES_ELI_CACHE_DIR")
    if env:
        return Path(env).expanduser()
    return Path.home() / ".matematic" / "cache" / "es-eli"
