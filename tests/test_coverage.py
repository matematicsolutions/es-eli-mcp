...
from es_eli_mcp.coverage import build_coverage
from es_eli_mcp.server import es_coverage


def _coverage():
    fn = getattr(es_coverage, "fn", es_coverage)
...
def test_tool_writes_an_audit_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ES_ELI_AUDIT_DIR", str(tmp_path))
    _coverage()
    lines = [
        line
    ...
    assert any("es_coverage" in line for line in lines), (
        "an audit entry was written but it does not name es_coverage"
    )
